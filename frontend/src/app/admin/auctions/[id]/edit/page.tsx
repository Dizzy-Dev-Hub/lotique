...
export default function EditAuctionPage() {
  ...
  }, [isAuthenticated, user, router, auctionId]);

  const loadAuction = async () => {
    try {
      const auction = await auctionsApi.getByIdAdmin(auctionId);
      setForm({
        title: auction.title || '',
        description: auction.description || '',
        category: auction.category || 'Jewelry',
        startingBid: auction.startingBid || 1000,
        reservePrice: auction.reservePrice || 0,
        buyNowPrice: auction.buyNowPrice || 0,
        scheduledStart: formatDateForInput(auction.startsAt || auction.endsAt),
        scheduledEnd: formatDateForInput(auction.endsAt),
        images: auction.images || [],
        condition: auction.conditionReport?.summary || 'Excellent',
        provenance: auction.conditionReport?.detailedDescription || '',
        dimensions: (auction.specifications as any)?.dimensions || '',
        weight: (auction.specifications as any)?.weight || '',
        materials: (auction.specifications as any)?.materials || '',
        yearCreated: (auction.specifications as any)?.yearCreated || '',
        artist: (auction.specifications as any)?.artist || '',
        isPrivate: auction.visibility === 'private',
        requiresDeposit: (auction.depositRequired || 0) > 0,
        depositAmount: auction.depositRequired || 1000,
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card className="p-6">
            ...
            <select
              ...
              className="w-full max-w-xs px-4 py-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)]"
            >
              {STATUSES.map(status => (
